...
func TestNormalizeProtocol(t *testing.T) {
	tests := []struct {
		...
		{"canonical host-agent is idempotent", "host-agent", "host-agent"},
		{"anthropic case-insensitive", "ANTHROPIC", ProtocolAnthropic},
		{"host-agent case-insensitive", "Host-Agent", "host-agent"},
...
func TestValidateProtocol(t *testing.T) {
	tests := []struct {
		...
		{"host-agent ok", "host-agent", false, ""},
...
func TestValidateProtocol_ErrorMessageListsAllProtocols(t *testing.T) {
	...
	for _, sub := range []string{ProtocolAnthropic, ProtocolOpenAIChatCompletions, ProtocolOpenAIResponses, ProtocolAnthropicBedrock, "host-agent"} {
